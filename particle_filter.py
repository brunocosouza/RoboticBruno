...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        res = robot()
        length = self.length # length of robot
        res.bearing_noise = self.bearing_noise # initialize bearing noise to zero
        res.steering_noise = self.steering_noise # initialize steering noise to zero
        res.distance_noise = self.distance_noise # initialize distance noise to zero
        ...
        turning_noise = random.gauss(turning, self.steering_noise)
        distance_noise = random.gauss(distance, self.distance_noise)
        
        turn = tan(turning_noise)*distance_noise/length
        
        if abs(turn) < tolerance:
            res.x = self.x + distance_noise*cos(self.orientation)
            res.y = self.y + distance_noise*sin(self.orientation)
            res.orientation = self.orientation + turn
        else :
            R = distance_noise/turn
            cx = self.x - sin(self.orientation)*R
            cy = self.y + cos(self.orientation)*R
            
            res.x = cx + sin(self.orientation + turn)*R
            res.y = cy - cos(self.orientation + turn)*R
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            ...
            gOrientation = atan2(deltaY,deltaX)
            
            bearing = gOrientation - self.orientation
            if add_noise:
